...
import { getBins, getMyFestivalBins, unAssignBinFromFestival, getFavoriteFestival, stopUsingFestivalBin, getUserRole } from '../api';
...
const MagicBins = () => {
  ...
  const fetchMyRole = async () => {
    try {
      ...
      console.log('FavoriteFestival.id', FavoriteFestival.id);
            ...
            <>
              ...
              <div className="w-full max-h-screen overflow-y-auto px-2 space-y-4 sm:pt-24 pb-44 pt-24 sm:pb-6 self-start">
                ...
                {bins
                  ...
                  .map((bin, index) => (
                    <Slide key={index} in={true} direction="right" timeout={100 + index * 100} mountOnEnter unmountOnExit>
                      <div>
                        <BinListElement
                          key={index}
                          title={bin.name}
              ...
              {unassignMode && (
                <div className="fixed flex flex-row space-x-2 bottom-32 left-1/2 transform -translate-x-1/2 z-10">
